...
#define   SWITCH_TYPE      05    //双直流电机584转 120转 17档

#define   CONFIG_PARA_DATA_LEN        38
...
typedef struct 
{
    ...
    uint16_t code;                       //设备编号
    //电容阈值
    uint16_t pfUpLimit;                  //功率因数上限 100倍系数
    uint16_t pfDownLimit;                //功率因数下限 100倍系数
    uint16_t thQNeed;                    //无功需求阈值 单位千乏
    uint16_t tcDelay;                    //投电容阈值 
    uint16_t qcDelay;                    //切电容阈值
    uint16_t crc; 

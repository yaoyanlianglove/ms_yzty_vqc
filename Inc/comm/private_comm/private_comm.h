...
#define SEND_HEART_LEN          26
#define SEND_GET_CFG_LEN        56  
...
typedef struct 
{
    ...
    uint16_t ykReturnTimeCounter;         //遥控返回计时

    uint16_t *deviceStatus;               //设备状态
    uint8_t  *capStatus;                  //电容器组状态
    uint8_t  *capFault;                   //电容器组故障
    uint8_t  *currentGear;                //当前档位
    uint8_t  *memoryGear;                 //记忆档位
    uint8_t  *coilReg[32];                //线圈寄存器
    uint8_t  *discreteInputReg[64];       //离散输入寄存器

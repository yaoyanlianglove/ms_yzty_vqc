...
StorageStatusTypeDef Save_Config_Para(ConfigParaTypeDef *cfgPara)
{
    ...
    writeData[20] = (cfgPara->thAlarmTemp >>8) & 0xFF;
    writeData[21] =  cfgPara->thAlarmTemp      & 0xFF;
    writeData[22] = (cfgPara->tranCapacity >>8) & 0xFF;
    writeData[23] =  cfgPara->tranCapacity      & 0xFF;
    
    writeData[24] = (cfgPara->code >>8) & 0xFF;
    writeData[25] =  cfgPara->code      & 0xFF;

    writeData[26] = (cfgPara->pfUpLimit >>8) & 0xFF;
    writeData[27] =  cfgPara->pfUpLimit      & 0xFF;
    writeData[28] = (cfgPara->pfDownLimit >>8) & 0xFF;
    writeData[29] =  cfgPara->pfDownLimit      & 0xFF;
    writeData[30] = (cfgPara->thQNeed >>8) & 0xFF;
    writeData[31] =  cfgPara->thQNeed      & 0xFF;
    writeData[32] = (cfgPara->tcDelay >>8) & 0xFF;
    writeData[33] =  cfgPara->tcDelay      & 0xFF;
    writeData[34] = (cfgPara->qcDelay >>8) & 0xFF;
    writeData[35] =  cfgPara->qcDelay      & 0xFF;

    cfgPara->crc = CRC_16(writeData, CONFIG_PARA_DATA_LEN - 2);
    writeData[CONFIG_PARA_DATA_LEN - 2] = (cfgPara->crc >> 8) & 0xFF;
    writeData[CONFIG_PARA_DATA_LEN - 1] =  cfgPara->crc       & 0xFF;
...
/*****************************************************************************
 Function    : Save_CAP1_Motion_Counter
 Description : None
 Input       : None
 Output      : None
 Return      : Save status.
 *****************************************************************************/
StorageStatusTypeDef Save_CAP1_Motion_Counter(void)
{
    FRAM_StatusTypeDef  res = FRAM_WRITE_READ_OK;
    uint8_t  writeData[4] = {0};
    uint16_t motionNum    = 0;
    uint16_t crc          = 0;

    res = FRAM_Read_With_CRC(CAP1_MOTION_COUNTER_ADDR, writeData, 4);
    if(res != FRAM_WRITE_READ_OK)
        return STORAGE_READ_ERROR;
    motionNum = (writeData[0] << 8) + writeData[1];
    if(motionNum < 0xFFFF)
        motionNum ++;
    writeData[0] = motionNum >> 8;
    writeData[1] = motionNum & 0xFF;
    crc = CRC_16(writeData, 2);
    writeData[2] = crc >> 8;
    writeData[3] = crc & 0xFF;
    res = FRAM_Write_With_CRC(CAP1_MOTION_COUNTER_ADDR, writeData, 4);
    if(res != FRAM_WRITE_READ_OK)
        return STORAGE_WRITE_ERROR;
    return STORAGE_OK;
}
/*****************************************************************************
 Function    : Save_CAP2_Motion_Counter
 Description : None
 Input       : None
 Output      : None
 Return      : Save status.
 *****************************************************************************/
StorageStatusTypeDef Save_CAP2_Motion_Counter(void)
{
    FRAM_StatusTypeDef  res = FRAM_WRITE_READ_OK;
    uint8_t  writeData[4] = {0};
    uint16_t motionNum    = 0;
    ...
    res = FRAM_Read_With_CRC(CAP2_MOTION_COUNTER_ADDR, writeData, 4);
    if(res != FRAM_WRITE_READ_OK)
        return STORAGE_READ_ERROR;
    motionNum = (writeData[0] << 8) + writeData[1];
    if(motionNum < 0xFFFF)
    ...
    writeData[2] = crc >> 8;
    writeData[3] = crc & 0xFF;
    res = FRAM_Write_With_CRC(CAP2_MOTION_COUNTER_ADDR, writeData, 4);
    if(res != FRAM_WRITE_READ_OK)
        return STORAGE_WRITE_ERROR;
    return STORAGE_OK;
}
                                        ...
                                        uint8_t               mode)
{
    ...
    writeData[6 ] = ((uint16_t)(beforeMotionSample->ua)) >> 8;
    writeData[7 ] = ((uint16_t)(beforeMotionSample->ua)) & 0xFF;
    writeData[8 ] = ((uint16_t)(beforeMotionSample->ub)) >> 8;
    writeData[9 ] = ((uint16_t)(beforeMotionSample->ub)) & 0xFF;
    writeData[10] = ((uint16_t)(beforeMotionSample->uc)) >> 8;
    writeData[11] = ((uint16_t)(beforeMotionSample->uc)) & 0xFF;

    writeData[12] = ((uint16_t)(beforeMotionSample->ia * 10.0)) >> 8;
    writeData[13] = ((uint16_t)(beforeMotionSample->ia * 10.0)) & 0xFF;
    writeData[14] = ((uint16_t)(beforeMotionSample->ib * 10.0)) >> 8;
    writeData[15] = ((uint16_t)(beforeMotionSample->ib * 10.0)) & 0xFF;
    writeData[16] = ((uint16_t)(beforeMotionSample->ic * 10.0)) >> 8;
    writeData[17] = ((uint16_t)(beforeMotionSample->ic * 10.0)) & 0xFF;

    writeData[18] = (uint16_t)(beforeMotionSample->pf * 100.0);

    writeData[19] = ((sw->beforeMotionCapStat) << 6) | ((sw->beforeMotionGear) << 1);

    writeData[20] = ((uint16_t)(sample->ua)) >> 8;
    writeData[21] = ((uint16_t)(sample->ua)) & 0xFF;
    writeData[22] = ((uint16_t)(sample->ub)) >> 8;
    writeData[23] = ((uint16_t)(sample->ub)) & 0xFF;
    writeData[24] = ((uint16_t)(sample->uc)) >> 8;
    writeData[25] = ((uint16_t)(sample->uc)) & 0xFF;

    writeData[26] = ((uint16_t)(sample->ia * 10.0)) >> 8;
    writeData[27] = ((uint16_t)(sample->ia * 10.0)) & 0xFF;
    writeData[28] = ((uint16_t)(sample->ib * 10.0)) >> 8;
    writeData[29] = ((uint16_t)(sample->ib * 10.0)) & 0xFF;
    writeData[30] = ((uint16_t)(sample->ic * 10.0)) >> 8;
    writeData[31] = ((uint16_t)(sample->ic * 10.0)) & 0xFF;

    writeData[32] = (uint16_t)(sample->pf * 100.0);
    writeData[33] = ((sw->currentCapStat) << 6) | ((sw->currentGear)<< 1);
    ...
    {
        if(sw->cap1MotionFlag == 1)
            Save_CAP1_Motion_Counter();
        if(sw->cap2MotionFlag == 1)
            Save_CAP2_Motion_Counter();
        sw->cap1MotionFlag = 0;
        sw->cap2MotionFlag = 0;
    }
    return STORAGE_OK;
                                ...
                                uint8_t               mode)
{
    ...
    writeData[6 ] = ((uint16_t)(sample->ua)) >> 8;
    writeData[7 ] = ((uint16_t)(sample->ua)) & 0xFF;
    writeData[8 ] = ((uint16_t)(sample->ub)) >> 8;
    writeData[9 ] = ((uint16_t)(sample->ub)) & 0xFF;
    writeData[10] = ((uint16_t)(sample->uc)) >> 8;
    writeData[11] = ((uint16_t)(sample->uc)) & 0xFF;

    writeData[12] = ((uint16_t)(sample->ia * 10.0)) >> 8;
    writeData[13] = ((uint16_t)(sample->ia * 10.0)) & 0xFF;
    writeData[14] = ((uint16_t)(sample->ib * 10.0)) >> 8;
    writeData[15] = ((uint16_t)(sample->ib * 10.0)) & 0xFF;
    writeData[16] = ((uint16_t)(sample->ic * 10.0)) >> 8;
    writeData[17] = ((uint16_t)(sample->ic * 10.0)) & 0xFF;
    

    writeData[18] = ((sw->currentCapStat) << 6) | ((sw->currentGear)<< 1);
    writeData[19] = mode;
...
StorageStatusTypeDef Save_Default_Config(ConfigParaTypeDef *cfgPara)
{
    ...
    cfgPara->thDownVoltage        = 10200;
    cfgPara->thUpVoltage          = 9800;
    cfgPara->tySpace              = 5; 
    cfgPara->tyDelay              = 300;
    cfgPara->ctRatio              = 300; 
    cfgPara->lockOverCurrent      = 400;
    cfgPara->lockVoltageUpLimit   = 13000;
    cfgPara->lockVoltageLowLimit  = 7000;
    cfgPara->lockHighVoltage      = 15000;
    cfgPara->lockLowVoltage       = 5000;
    cfgPara->thAlarmTemp          = 60;
    cfgPara->tranCapacity         = 400;
    cfgPara->code                 = 01;

    cfgPara->pfUpLimit            = 95;
    cfgPara->pfDownLimit          = 85;
    cfgPara->thQNeed              = 5;
    cfgPara->tcDelay              = 300;
    cfgPara->qcDelay              = 300;
...
StorageStatusTypeDef Read_Config_Para(ConfigParaTypeDef *cfgPara)
{
    ...
    cfgPara->thAlarmTemp          = (readData[20]<< 8)  +  readData[21];
    cfgPara->tranCapacity         = (readData[22]<< 8)  +  readData[23];
    cfgPara->code                 = (readData[24]<< 8)  +  readData[25];
    cfgPara->pfUpLimit            = (readData[26]<< 8)  +  readData[27];
    cfgPara->pfDownLimit          = (readData[28]<< 8)  +  readData[29];
    cfgPara->thQNeed              = (readData[30]<< 8)  +  readData[31];
    cfgPara->tcDelay              = (readData[32]<< 8)  +  readData[33];
    cfgPara->qcDelay              = (readData[34]<< 8)  +  readData[35];
    cfgPara->crc                  = (readData[36]<< 8)  +  readData[37];
    return STORAGE_OK;
}
/*****************************************************************************
...
/*****************************************************************************
 Function    : Read_CAP1_Motion_Counter
 Description : None
 Input       : data-The pointer of array.
 Output      : None
 Return      : Read status.
 *****************************************************************************/
StorageStatusTypeDef Read_CAP1_Motion_Counter(uint8_t *data)
{
    if(FRAM_Read_With_CRC(CAP1_MOTION_COUNTER_ADDR, data, 4) != FRAM_WRITE_READ_OK)
        return STORAGE_READ_ERROR;
    return STORAGE_OK;
}
/*****************************************************************************
 Function    : Read_CAP2_Motion_Counter
 Description : None
 Input       : data-The pointer of array.
 Output      : None
 Return      : Read status.
 *****************************************************************************/
StorageStatusTypeDef Read_CAP2_Motion_Counter(uint8_t *data)
{
    if(FRAM_Read_With_CRC(CAP2_MOTION_COUNTER_ADDR, data, 4) != FRAM_WRITE_READ_OK)
        return STORAGE_READ_ERROR;
    return STORAGE_OK;
}
/*****************************************************************************

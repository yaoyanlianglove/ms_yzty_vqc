...
uint8_t  goToMainTapFlag = 1;                    //上电后执行回主分接头标志 初始化为0，表示执行此功能，初始化为1，表示跳过此功能
/***********设备状态开始************************************************************/
...
void YZTY_Read_Telemetry(void)
{
    ...
    {
        g_telemetry.sample.ua = g_telemetry.sample.ua * 25 * 1.732;
        g_telemetry.sample.ub = g_telemetry.sample.ub * 25 * 1.732;
        g_telemetry.sample.uc = g_telemetry.sample.uc * 25 * 1.732;
        g_telemetry.sample.ia = g_telemetry.sample.ia * g_configPara.ctRatio;
        g_telemetry.sample.ib = g_telemetry.sample.ib * g_configPara.ctRatio;
        g_telemetry.sample.ic = g_telemetry.sample.ic * g_configPara.ctRatio; 
        g_telemetry.sample.pa = g_telemetry.sample.pa * 25 * g_configPara.ctRatio / 1000;
        g_telemetry.sample.pb = g_telemetry.sample.pb * 25 * g_configPara.ctRatio / 1000;
        g_telemetry.sample.pc = g_telemetry.sample.pc * 25 * g_configPara.ctRatio / 1000;
        g_telemetry.sample.qa = g_telemetry.sample.qa * 25 * g_configPara.ctRatio / 1000;
        g_telemetry.sample.qb = g_telemetry.sample.qb * 25 * g_configPara.ctRatio / 1000;
        g_telemetry.sample.qc = g_telemetry.sample.qc * 25 * g_configPara.ctRatio / 1000;

        telemetryReg[0 ] =  (int16_t)(g_telemetry.sample.ua);
        telemetryReg[1 ] =  (int16_t)(g_telemetry.sample.ub);
        telemetryReg[2 ] =  (int16_t)(g_telemetry.sample.uc);
        telemetryReg[3 ] =  (int16_t)(g_telemetry.sample.ia * 10);
        telemetryReg[4 ] =  (int16_t)(g_telemetry.sample.ib * 10);
        telemetryReg[5 ] =  (int16_t)(g_telemetry.sample.ic * 10);
        telemetryReg[6 ] =  (int16_t)(g_telemetry.sample.pa * 10);
        telemetryReg[7 ] =  (int16_t)(g_telemetry.sample.pb * 10);
        telemetryReg[8 ] =  (int16_t)(g_telemetry.sample.pc * 10);
        telemetryReg[9 ] =  (int16_t)(g_telemetry.sample.qa * 10);
        telemetryReg[10] =  (int16_t)(g_telemetry.sample.qb * 10);
        telemetryReg[11] =  (int16_t)(g_telemetry.sample.qc * 10);
...
void YZTY_Time_Counter(void)
{
    ...
    else
    {
        ...
        if(tySpace > 0)
            tySpace--;
        Auto_Control_Time_Counter(&g_switch, g_configPara.tyDelay, g_configPara.tcDelay, g_configPara.qcDelay);
    ...
    //以记忆挡位为准
    Led_Control_Counter(g_deviceStatus.lockSwitch, g_switch.memoryGear, g_switch.currentCapStat);
...
void YZTY_Device_Status_Judge(void)
{
    ...
    g_deviceStatus.switchFault    = g_remoteSignal.switchFault      ;         
    ...
    g_deviceStatus.controlCapFail = g_remoteSignal.controlCapFail   ; 
    g_deviceStatus.cap1Fault      = g_remoteSignal.cap1Fault        ;
    g_deviceStatus.cap2Fault      = g_remoteSignal.cap2Fault        ; 
...
{
    if(g_telemetry.sample.ia > g_configPara.lockOverCurrent 
        || g_telemetry.sample.ib > g_configPara.lockOverCurrent 
        || g_telemetry.sample.ic > g_configPara.lockOverCurrent)
        overCurrentFlag = 1;   
    else
        overCurrentFlag = 0;
...
void YZTY_Lock_Judge(void)
{
    if(g_remoteSignal.overCurrentAlarm == 1 || g_remoteSignal.highVoltageAlarm == 1 || 
       ...
       g_remoteSignal.switchFault      == 1 || g_remoteSignal.cap1Fault        == 1 ||
       g_remoteSignal.cap2Fault        == 1)
...
{
    CapStatusTypeDef res = CAP_OK;
    if(Gear_Check(&g_switch, &g_remoteSignal.gearSignal) == GEAR_OK)
        g_remoteSignal.gearFault  = 0;
    else
        g_remoteSignal.gearFault  = 1;
    res = Cap_Check(&g_switch, &g_remoteSignal.capSignal);
    if(res != CAP_OK)
    {
        if(res == CAP1_AND_CAP2_ERROR)
        {
            g_remoteSignal.cap1Fault  = 1;
            g_remoteSignal.cap2Fault  = 1;
        }
        else if(res == CAP1_ERROR)
            g_remoteSignal.cap1Fault  = 1;
        else
            g_remoteSignal.cap2Fault  = 1;
    }
...
/*****************************************************************************
 Function    : YZTY_Cap_Action
 Description : None
 Input       : None
 Output      : None
 Return      : None
 *****************************************************************************/
uint8_t YZTY_Cap_Action(void)
{
    CapStatusTypeDef res = CAP_OK;

    /*-----------------关闭中断------------------------*/
    __disable_irq();
    res = Cap_Control(&g_switch);
    /*-----------------打开中断------------------------*/
    __enable_irq();
    if(res != CAP_OK)
    {
...
void YZTY_Control_Judge(void)
{
    ...
    {
        g_switch.autoMotion = Auto_Control(&g_telemetry.sample, &g_configPara, &g_switch);
    }
    if(g_remoteSignal.lockSwitch == 0)
    {
        ...
        else if(g_switch.motion == RC_CONTROL_CAP)
        {
            action = RC_CONTROL_CAP;
        }      
        else
            action = RC_NONE;
    }
    ...
    else if(action == RC_CONTROL_CAP)
    {
        if(tySpace == 0)
        {
            if(YZTY_Cap_Action() != 0)
                g_remoteSignal.controlCapFail = 1;
            else
                g_remoteSignal.controlCapFail = 0;
            g_switch.lastMotion     = g_switch.motion;
            g_saveFlag.switchMotionFlag = 1;
            tySpace = g_configPara.tySpace;
            if(g_remoteSignal.autoMode == 1)
...
void YZTY_Storage(void)
{
    ...
    g_switch.beforeMotionCapStat = g_switch.currentCapStat;
    lastDeciveStatus = currentDeciveStatus;

...
/*****************************************************************************
 ...
 Return      : None
 *****************************************************************************/
void Led_Control_Counter(uint8_t deviceStatus, uint8_t gear, uint8_t capStat)
{
    ...
    if(ledMSTimeCounter < 1000)
    {
        ...
        if(ledMSTimeCounter == 400)
        {
            ...
            if(ledSecTimeCounter < LED_ALARM_CONTROL_PERIOD)
                ...
                ledSecTimeCounter < (LED_ALARM_CONTROL_PERIOD + capStat))
            {
                Led_Red_On();
            }
            else if(ledSecTimeCounter < LED_ALARM_CONTROL_PERIOD + capStat + gear)
                Led_Green_On();
        }
    ...
    {
        ledMSTimeCounter = 0;
        if(ledSecTimeCounter < LED_ALARM_CONTROL_PERIOD + capStat + gear)
            ledSecTimeCounter++;
        else
            ledSecTimeCounter = 0;
    }
